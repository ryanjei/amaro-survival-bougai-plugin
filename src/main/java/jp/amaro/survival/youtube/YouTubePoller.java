...
public final class YouTubePoller implements YouTubeRuntime.Connection {
    ...
    private final Set<String> seenIds = new LinkedHashSet<>(); private volatile boolean closed; private boolean initialized; private String pageToken; private Instant lastErrorLog = Instant.EPOCH;
    public YouTubePoller(YouTubeCommentSource client, Consumer<YouTubeComment> sink, Logger logger) { this.client = client; this.sink = sink; this.logger = logger; }
    ...
    private void poll() {
        if (closed) return;
        try {
            YouTubePollResult result = client.poll(pageToken); pageToken = result.nextPageToken();
            if (!initialized) { for (YouTubeComment c : result.comments()) remember(c.id()); initialized = true; }
            else for (YouTubeComment c : result.comments()) if (remember(c.id())) sink.accept(c);
            schedule(result.nextPollDelay());
        }
